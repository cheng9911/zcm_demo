...
const __example_zcmtypes_multidim_t_hash = Ref(Int64(0))
function ZCM._get_hash_recursive(::Type{multidim_t}, parents::Array{String})
    if __example_zcmtypes_multidim_t_hash[] != 0; return __example_zcmtypes_multidim_t_hash[]; end
    if "example_zcmtypes_multidim_t" in parents; return 0; end
    hash::UInt64 = 0xe450cfbc53e83411
    hash = (hash << 1) + ((hash >>> 63) & 0x01)
    __example_zcmtypes_multidim_t_hash[] = reinterpret(Int64, hash)
...
function ZCM._encode_one(msg::multidim_t, buf)
    ...
    @assert size(msg.mat,1)==msg.rows "Msg of type `example.zcmtypes.multidim_t` requires field `mat` dimension `1` to be size `msg.rows`"
    @assert size(msg.mat,2)==2 "Msg of type `example.zcmtypes.multidim_t` requires field `mat` dimension `2` to be size `2`"
    @assert size(msg.mat,3)==msg.jk "Msg of type `example.zcmtypes.multidim_t` requires field `mat` dimension `3` to be size `msg.jk`"
    for i0=1:msg.rows
        for i1=1:2
            for i2=1:msg.jk
...
function ZCM._decode_one(::Type{multidim_t}, buf)
    ...
    @static if VERSION < v"0.7.0-"
        msg.mat = Array{Float64, 3}(msg.rows, 2, msg.jk)
    else
...
function ZCM.fieldnames(::Type{multidim_t})
    return (
        :rows,
        :jk,
        :mat,
...
function ZCM.constfieldnames(::Type{multidim_t})
    return (
    )
end
