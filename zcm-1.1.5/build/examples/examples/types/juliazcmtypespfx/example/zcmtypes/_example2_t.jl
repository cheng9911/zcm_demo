...
const __example_zcmtypes_example2_t_hash = Ref(Int64(0))
function ZCM._get_hash_recursive(::Type{example2_t}, parents::Array{String})
    if __example_zcmtypes_example2_t_hash[] != 0; return __example_zcmtypes_example2_t_hash[]; end
    if "example_zcmtypes_example2_t" in parents; return 0; end
    hash::UInt64 = 0x2caa13979d1b6e97
    hash = (hash << 1) + ((hash >>> 63) & 0x01)
    __example_zcmtypes_example2_t_hash[] = reinterpret(Int64, hash)
...
function ZCM._encode_one(msg::example2_t, buf)
    ...
    @assert size(msg.position2,1)==3 "Msg of type `example.zcmtypes.example2_t` requires field `position2` dimension `1` to be size `3`"
    for i0=1:3
        write(buf, hton(msg.position2[i0]))
    end
    @assert size(msg.orientation2,1)==4 "Msg of type `example.zcmtypes.example2_t` requires field `orientation2` dimension `1` to be size `4`"
    for i0=1:4
        write(buf, hton(msg.orientation2[i0]))
    end
    write(buf, hton(msg.num_ranges2))
    @assert size(msg.ranges2,1)==msg.num_ranges2 "Msg of type `example.zcmtypes.example2_t` requires field `ranges2` dimension `1` to be size `msg.num_ranges2`"
...
function ZCM._decode_one(::Type{example2_t}, buf)
    ...
    @static if VERSION < v"0.7.0-"
        msg.position2 = Array{Float64, 1}(3)
    else
    ...
    @static if VERSION < v"0.7.0-"
        msg.orientation2 = Array{Float64, 1}(4)
    else
    ...
    @static if VERSION < v"0.7.0-"
        msg.ranges2 = Array{Int16, 1}(msg.num_ranges2)
    else
...
function ZCM.fieldnames(::Type{example2_t})
    return (
        :timestamp2,
        :position2,
        :orientation2,
...
function ZCM.constfieldnames(::Type{example2_t})
    return (
    )
end
