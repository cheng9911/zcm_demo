...
mutable struct example_t <: ZCM.AbstractZcmType
    ...
    nExamples1                    ::Int32
    nExamples2                    ::Int32
    subExamples                   ::Array{ZCM.AbstractZcmType,2} # juliazcmtypespfx.example.zcmtypes.example_t
    subStrings                    ::Array{String,2}
    bitfield1                     ::Int8
    bitfield2                     ::Array{Int8,1}
    ...
    function example_t()
        ...
        self.nExamples1 = 0
        self.nExamples2 = 0
        self.subExamples = [ __basemodule.example.zcmtypes.example_t() for dim0=1:self.nExamples1, dim1=1:self.nExamples2 ]
        self.subStrings = [ "" for dim0=1:self.nExamples1, dim1=1:self.nExamples2 ]
        self.bitfield1 = 0
        self.bitfield2 = [ 0 for dim0=1:4 ]
        ...
        self.test_const_string = "example"
...
const __example_zcmtypes_example_t_hash = Ref(Int64(0))
function ZCM._get_hash_recursive(::Type{example_t}, parents::Array{String})
    if __example_zcmtypes_example_t_hash[] != 0; return __example_zcmtypes_example_t_hash[]; end
    if "example_zcmtypes_example_t" in parents; return 0; end
    newparents::Array{String} = [parents[:]; "example_zcmtypes_example_t"::String];
    hash::UInt64 = 0x1939f4d104c8d2f8 + reinterpret(UInt64, ZCM._get_hash_recursive(__basemodule.example.zcmtypes.example_t, newparents))
    hash = (hash << 1) + ((hash >>> 63) & 0x01)
    __example_zcmtypes_example_t_hash[] = reinterpret(Int64, hash)
...
function ZCM._encode_one(msg::example_t, buf)
    ...
    @assert size(msg.position,1)==3 "Msg of type `example.zcmtypes.example_t` requires field `position` dimension `1` to be size `3`"
    for i0=1:3
        write(buf, hton(msg.position[i0]))
    end
    @assert size(msg.orientation,1)==4 "Msg of type `example.zcmtypes.example_t` requires field `orientation` dimension `1` to be size `4`"
    for i0=1:4
        write(buf, hton(msg.orientation[i0]))
    end
    write(buf, hton(msg.num_ranges))
    @assert size(msg.ranges,1)==msg.num_ranges "Msg of type `example.zcmtypes.example_t` requires field `ranges` dimension `1` to be size `msg.num_ranges`"
    ...
    @assert size(msg.subExamples,1)==msg.nExamples1 "Msg of type `example.zcmtypes.example_t` requires field `subExamples` dimension `1` to be size `msg.nExamples1`"
    @assert size(msg.subExamples,2)==msg.nExamples2 "Msg of type `example.zcmtypes.example_t` requires field `subExamples` dimension `2` to be size `msg.nExamples2`"
    for i0=1:msg.nExamples1
        for i1=1:msg.nExamples2
            @assert isa(msg.subExamples[i0,i1], __basemodule.example.zcmtypes.example_t) "Msg of type `example.zcmtypes.example_t` requires field `msg.subExamples[i0,i1]` to be of type `example.zcmtypes.example_t`"
            ZCM._encode_one(msg.subExamples[i0,i1],buf)
        end
    end
    @assert size(msg.subStrings,1)==msg.nExamples1 "Msg of type `example.zcmtypes.example_t` requires field `subStrings` dimension `1` to be size `msg.nExamples1`"
    @assert size(msg.subStrings,2)==msg.nExamples2 "Msg of type `example.zcmtypes.example_t` requires field `subStrings` dimension `2` to be size `msg.nExamples2`"
    for i0=1:msg.nExamples1
        for i1=1:msg.nExamples2
    ...
    byte_in_progress, offset_bit = ZCM.write_bits(buf, msg.bitfield1, 2, byte_in_progress, offset_bit)
    @assert size(msg.bitfield2,1)==4 "Msg of type `example.zcmtypes.example_t` requires field `bitfield2` dimension `1` to be size `4`"
    for i0=1:4
        byte_in_progress, offset_bit = ZCM.write_bits(buf, msg.bitfield2[i0], 3, byte_in_progress, offset_bit)
    end
    if (offset_bit != 0)
...
function ZCM._decode_one(::Type{example_t}, buf)
    ...
    @static if VERSION < v"0.7.0-"
        msg.subExamples = Array{ZCM.AbstractZcmType, 2}(msg.nExamples1, msg.nExamples2)
    else
        msg.subExamples = Array{ZCM.AbstractZcmType, 2}(undef, msg.nExamples1, msg.nExamples2)
    end
    for i0=1:msg.nExamples1
        for i1=1:msg.nExamples2
            msg.subExamples[i0,i1] = ZCM._decode_one(__basemodule.example.zcmtypes.example_t,buf)
        end
    end
    @static if VERSION < v"0.7.0-"
        msg.subStrings = Array{String, 2}(msg.nExamples1, msg.nExamples2)
    else
    ...
    # Start of bitfield 0
    offset_bit = 0
    offset_bit, msg.bitfield1 = ZCM.read_bits(Int8, buf, 2, offset_bit, false)
    @static if VERSION < v"0.7.0-"
        msg.bitfield2 = Array{Int8, 1}(4)
    else
        msg.bitfield2 = Array{Int8, 1}(undef, 4)
    end
    for i0=1:4
        offset_bit, msg.bitfield2[i0] = ZCM.read_bits(Int8, buf, 3, offset_bit, false)
    end
    if (offset_bit != 0)
        read(buf, 1)
    end
...
function ZCM.fieldnames(::Type{example_t})
    return (
        :timestamp,
        :position,
        :orientation,
...
function ZCM.constfieldnames(::Type{example_t})
    return (
