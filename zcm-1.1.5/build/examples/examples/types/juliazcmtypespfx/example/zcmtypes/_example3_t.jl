...
const __example_zcmtypes_example3_t_hash = Ref(Int64(0))
function ZCM._get_hash_recursive(::Type{example3_t}, parents::Array{String})
    if __example_zcmtypes_example3_t_hash[] != 0; return __example_zcmtypes_example3_t_hash[]; end
    if "example_zcmtypes_example3_t" in parents; return 0; end
    hash::UInt64 = 0x3d4bc295e6734beb
    hash = (hash << 1) + ((hash >>> 63) & 0x01)
    __example_zcmtypes_example3_t_hash[] = reinterpret(Int64, hash)
...
function ZCM._encode_one(msg::example3_t, buf)
    @assert size(msg.arr,1)==10 "Msg of type `example.zcmtypes.example3_t` requires field `arr` dimension `1` to be size `10`"
    for i0=1:10
