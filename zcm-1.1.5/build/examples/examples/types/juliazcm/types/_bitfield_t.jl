...
const __bitfield_t_hash = Ref(Int64(0))
function ZCM._get_hash_recursive(::Type{bitfield_t}, parents::Array{String})
    if __bitfield_t_hash[] != 0; return __bitfield_t_hash[]; end
    if "bitfield_t" in parents; return 0; end
    hash::UInt64 = 0x4e73ecb0ea215bc2
    hash = (hash << 1) + ((hash >>> 63) & 0x01)
    __bitfield_t_hash[] = reinterpret(Int64, hash)
...
function ZCM._encode_one(msg::bitfield_t, buf)
    ...
    @assert size(msg.field2,1)==2 "Msg of type `bitfield_t` requires field `field2` dimension `1` to be size `2`"
    @assert size(msg.field2,2)==4 "Msg of type `bitfield_t` requires field `field2` dimension `2` to be size `4`"
    for i0=1:2
        for i1=1:4
    ...
    offset_bit = 0
    byte_in_progress = UInt8(0)
    @assert size(msg.field8,1)==msg.field8_dim1 "Msg of type `bitfield_t` requires field `field8` dimension `1` to be size `msg.field8_dim1`"
    @assert size(msg.field8,2)==msg.field8_dim2 "Msg of type `bitfield_t` requires field `field8` dimension `2` to be size `msg.field8_dim2`"
    ...
    @assert size(msg.field12,1)==3 "Msg of type `bitfield_t` requires field `field12` dimension `1` to be size `3`"
    @assert size(msg.field12,2)==2 "Msg of type `bitfield_t` requires field `field12` dimension `2` to be size `2`"
    @assert size(msg.field12,3)==2 "Msg of type `bitfield_t` requires field `field12` dimension `3` to be size `2`"
    @assert size(msg.field12,4)==2 "Msg of type `bitfield_t` requires field `field12` dimension `4` to be size `2`"
    for i0=1:3
        for i1=1:2
            for i2=1:2
                for i3=1:2
