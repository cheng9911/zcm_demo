...
@static if VERSION < v"0.7.0-"
    @assert endswith(string(current_module()), "juliazcm.types.foo") "Only import this module through its parent"
else
    @assert endswith(string(@__MODULE__), "juliazcm.types.foo") "Only import this module through its parent"
end
...
mutable struct t2 <: ZCM.AbstractZcmType
    ...
    x                             ::Int64
    y                             ::ZCM.AbstractZcmType # juliazcm.types.foo.bar.t3
    ...
    function t2()
        ...
        self.x = 0
        self.y = __basemodule.types.foo.bar.t3()
...
function ZCM._get_hash_recursive(::Type{t2}, parents::Array{String})
    ...
    hash::UInt64 = 0x28ded9c6b827f01b + reinterpret(UInt64, ZCM._get_hash_recursive(__basemodule.types.foo.bar.t3, newparents))
...
function ZCM._encode_one(msg::t2, buf)
    ...
    @assert isa(msg.y, __basemodule.types.foo.bar.t3) "Msg of type `foo.t2` requires field `y` to be of type `foo.bar.t3`"
...
function ZCM._decode_one(::Type{t2}, buf)
    ...
    msg.y = ZCM._decode_one(__basemodule.types.foo.bar.t3,buf)
    return msg
end
