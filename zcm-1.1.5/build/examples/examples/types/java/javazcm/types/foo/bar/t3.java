...
{
    public javazcm.types.unpackaged_t x;
    ...
    {
        if (classes.contains(javazcm.types.foo.bar.t3.class))
            return 0L;
 
        classes.add(javazcm.types.foo.bar.t3.class);
        long hash = ZCM_FINGERPRINT_BASE
             + javazcm.types.unpackaged_t._hashRecursive(classes)
            ;
    ...
    public static javazcm.types.foo.bar.t3 _decodeRecursiveFactory(ZCMDataInputStream ins) throws IOException
    {
        javazcm.types.foo.bar.t3 o = new javazcm.types.foo.bar.t3();
    ...
    {
        this.x = javazcm.types.unpackaged_t._decodeRecursiveFactory(ins);
 
    }
 
    public javazcm.types.foo.bar.t3 copy()
    {
        javazcm.types.foo.bar.t3 outobj = new javazcm.types.foo.bar.t3();
